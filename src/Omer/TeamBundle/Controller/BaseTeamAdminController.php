...
class BaseTeamAdminController extends CRUDController
{
    /**
        ...
        if ($form->isSubmitted()) {
            ...
            if ($isFormValid && (!$this->isInPreviewMode() || $this->isPreviewApproved())) {
                try {
                    ...
                    $this->removeUnlinkedItems('OmerTeamBundle:TeamMember');
                    $this->removeUnlinkedItems('OmerTeamBundle:OtherPeople');
                    $this->em->flush();
    ...
    private function removeUnlinkedItems($className)
    {
        $objects = $this->em->createQueryBuilder()
            ->select('o')
            ->from($className, 'o')
            ->where('o.team is null')
            ->getQuery()
            ->getResult()
        ;
